...
RESULTS_DIR = os.path.join(ROOT, "result1")
os.makedirs(RESULTS_DIR, exist_ok=True)
OUT_CSV = os.path.join(RESULTS_DIR, "robust_fgsm_metrics.csv")
OUT_PNG = os.path.join(RESULTS_DIR, "robust_acc_auc_vs_eps.png")
OUT_JSON = os.path.join(RESULTS_DIR, "robust_fgsm_summary.json")
...
def eval_one_eps(model, loader, eps):
    y_true_all = []
    y_score_all = []
    y_pred_all = []
    ...
    for x, y in tqdm(loader, desc=f"FGSM eps={eps:.5f}", leave=False):
        x = x.to(device, non_blocking=True)
        y = y.to(device, non_blocking=True)

        # FGSM needs grad → temporarily enable grad
        ...
        y_score_all.append(score_pos.detach().cpu())
...
def plot_curves(df, out_png):
    ...
    plt.ylabel("score")
    plt.title("FGSM Robustness on NIH Test Set")
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    model = build_model()
    ds, dl = make_loader()
    print(f"Test size: {len(ds)} | batch={BATCH_SIZE} | workers={NUM_WORKERS}")

    rows = []
    for eps in EPS_LIST:
        rows.append(eval_one_eps(model, dl, eps))

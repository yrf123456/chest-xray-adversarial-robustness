"""
FGSM Attack Evaluation Script.

Evaluates model robustness against FGSM (Fast Gradient Sign Method) attack
...
def evaluate(model, loader, eps):
    ...
    for x, y in tqdm(loader, desc=f"FGSM eps={eps_label(eps)}", leave=False):
...
def plot_confusion(y_true, y_pred, eps):
    ...
    ax.set_title(f"FGSM Confusion Matrix (ε={eps_label(eps)})")
    ax.set_xlabel("Predicted")
    ax.set_ylabel("True")
    fig.tight_layout()
    fig.savefig(os.path.join(RESULTS_DIR, f"fgsm_confusion_eps{eps}.png"),
...
def plot_roc(y_true, y_score, eps):
    ...
    ax.set_title(f"FGSM ROC Curve (ε={eps_label(eps)})")
    ax.legend(loc="lower right")
    fig.tight_layout()
    fig.savefig(os.path.join(RESULTS_DIR, f"fgsm_roc_eps{eps}.png"),
                dpi=200, bbox_inches="tight", pad_inches=0.1)
    plt.close(fig)
...
def plot_score_dist(y_true, y_score, eps):
    ...
    ax.set_title(f"FGSM Score Distribution (ε={eps_label(eps)})")
    fig.tight_layout()
    fig.savefig(os.path.join(RESULTS_DIR, f"fgsm_score_eps{eps}.png"),
                dpi=200, bbox_inches="tight", pad_inches=0.1)
    plt.close(fig)
...
def plot_reliability(y_true, y_score, eps):
    ...
    ax.set_title(f"FGSM Reliability Diagram (ε={eps_label(eps)})")
    ax.legend(loc="lower right")
    fig.tight_layout()
    fig.savefig(os.path.join(RESULTS_DIR, f"fgsm_reliability_eps{eps}.png"),
                dpi=200, bbox_inches="tight", pad_inches=0.1)
    plt.close(fig)


# ---------------- Main ----------------
def main(max_samples=None, eps_list=None):
    """Run FGSM evaluation."""
    ...
    for eps in eps_list:
        acc, auc, ece, y_true, y_pred, y_score = evaluate(model, loader, eps)
    ...
    ax.set_title("FGSM Robustness Curve", fontsize=12, fontweight='bold')
    ax.legend(loc="lower left")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(os.path.join(RESULTS_DIR, "fgsm_robustness_curve.png"),
                dpi=200, bbox_inches="tight", pad_inches=0.1)
    plt.close(fig)
    
    # Save results to JSON
    with open(os.path.join(RESULTS_DIR, "fgsm_eval_results.json"), "w") as f:
...
if __name__ == "__main__":
    ...
    main(max_samples=args.max_samples, eps_list=args.eps_subset)

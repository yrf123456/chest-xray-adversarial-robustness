...
We use the NIH ChestX-ray14 dataset \cite{wang2017} for several reasons:
\begin{itemize}[leftmargin=*]
...
We embed ImageNet normalization within the model's forward pass, ensuring adversarial perturbations are computed in the original [0, 1] pixel space for interpretable $\epsilon$ values.

\subsection{Adversarial Attack Methods}

We select \textbf{FGSM} and \textbf{PGD} as representative attacks for the following reasons:
\begin{itemize}[leftmargin=*]
...
We use the $L_\infty$-bounded threat model: $\|x_{adv} - x\|_\infty \leq \epsilon$. This is the standard choice in adversarial robustness research because it bounds per-pixel changes, directly relating to visual imperceptibility. We evaluate at $\epsilon \in \{0, 0.25/255, 0.5/255, 0.75/255, 1/255, 2/255\}$, spanning imperceptible to barely visible perturbations.
...
\end{equation}
We use 10 iterations with $\alpha = \epsilon/4$. PGD is considered one of the strongest first-order attacks and serves as a more rigorous robustness test than FGSM.

\subsection{Evaluation Metrics}

We use a comprehensive set of metrics to capture different aspects of model performance and robustness:
...
We use \textbf{Grad-CAM} \cite{selvaraju2017} to understand how adversarial attacks affect model attention. This choice is motivated by:
\begin{itemize}[leftmargin=*]
...
We target ResNet-18's final convolutional layer (layer4), producing 7$\times$7 heatmaps upsampled to input dimensions. Adversarial perturbations are visualized by computing $\delta = x_{adv} - x$ and amplifying for visibility.
...
\begin{appendices}
    ...
    A formal risk assessment was completed at the start of the project (see attached document: \texttt{SSEN Student Project Risk Assessment\_RuifengYan.docx}). The primary risks identified include:
    
    \begin{itemize}
        \item \textbf{Data Privacy:} The NIH ChestX-ray14 dataset is de-identified and publicly available, minimizing privacy risks.
        \item \textbf{Misuse Potential:} Research on adversarial attacks could theoretically inform malicious actors. However, this risk is mitigated by the public nature of existing attack methods and the importance of understanding vulnerabilities for defense development.
        \item \textbf{Clinical Misinterpretation:} Results should not be interpreted as indicating that all medical AI systems are unsafe, but rather as motivation for robustness testing.
    \end{itemize}
    
    \section{Ethics Form}
    \label{app:ethics}
    
    The DIICSU Honours Project Ethics Form was completed and submitted (see attached document: \texttt{DIICSU Honours Project Ethics Form——Ruifeng Yan.docx}). This project uses only publicly available, de-identified data (NIH ChestX-ray14) and does not involve human subjects research. No additional ethics approval was required.
    
    \section{Project Plan and Gantt Chart}
    \label{app:gantt}
    
    A detailed project plan was created using a Gantt chart (see attached document: \texttt{Project Gantt Chart -Ruifeng Yan.xlsx}). The project was completed in the following phases:
    
    \begin{enumerate}
        \item \textbf{Literature Review (Weeks 1-5):} Comprehensive survey of adversarial ML and medical imaging literature, covering FGSM, PGD, and C\&W attacks.
        \item \textbf{Prototype Experimentation (Weeks 6-10):} Implementation of miniature pipeline using a small chest X-ray dataset to validate core components.
        \item \textbf{Dataset Preparation (Weeks 11-13):} Download, preprocessing, and partitioning of NIH ChestX-ray14 dataset.
        \item \textbf{Model Training (Weeks 14-15):} Training and validation of baseline ResNet-18 model with ImageNet pre-training.
        \item \textbf{Attack Implementation (Weeks 16-17):} Implementation of FGSM and PGD adversarial attacks.
        \item \textbf{Robustness Evaluation (Weeks 18-20):} Comprehensive evaluation with accuracy-epsilon curves and quantitative metrics.
        \item \textbf{Visualization and Analysis (Weeks 21-23):} Grad-CAM visualization, adversarial example generation, and failure mode analysis.
        \item \textbf{Report Writing (Weeks 24-26):} Documentation and thesis writing.
        \item \textbf{Contingency Buffer (Weeks 27-28):} Reserved time for unexpected issues and final revisions.
    \end{enumerate}
    
    \section{Midterm Report}
    \label{app:midterm}
    
    A midterm report was submitted documenting progress at the project's midpoint. The report detailed:
    
    \begin{itemize}
        \item Project aims, objectives, and SMART goals
        \item Summary of completed work including literature review, environment setup, and prototype experimentation
        \item Initial results demonstrating unified L2 robustness curves for ResNet18 and DenseNet121
        \item Plan for remaining work and personal reflection
    \end{itemize}
    
    The full midterm report is included as a separate PDF document: \texttt{Honours Project Midterm Report - Ruifeng Yan.pdf}.
    ...
    \begin{itemize}
        \item \textbf{GitHub Copilot:} Used for code completion and debugging suggestions
        \item \textbf{ChatGPT/Claude:} Used for clarifying technical concepts and debugging assistance
    \end{itemize}
    
    All AI-generated content was reviewed, verified, and modified as necessary. The core research methodology, experimental design, and analysis were conducted independently.
    
    \section{Supervisor Meeting Minutes}
    
    Regular meetings were held with the supervisor throughout the project. Key discussion points included:
    
    \begin{itemize}
        \item Initial project scoping and objectives
        \item Dataset selection and preprocessing approach
        \item Attack method selection (FGSM vs PGD vs C\&W)
        \item Evaluation metric design
        \item Results interpretation and presentation
        \item Report structure and writing
    \end{itemize}
